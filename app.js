...
function calculateResults() {
  ...
  const categories = [
    {
      label: 'Community Fit',
      value: ['roots', 'balance'].includes(answers.priority) ? 'Excellent' : 'Strong'
    },
    {
      label: 'Professional Fit',
      value: ['professional', 'manager', 'executive'].includes(answers.career) ? 'Excellent' : 'Good'
    },
    {
      label: 'Lifestyle Fit',
      value: answers.mobility === 'high' ? 'Exceptional' : answers.mobility === 'medium' ? 'Strong' : 'Moderate'
    }
  ];
...
function showResults() {
  ...
  document.getElementById('resultBreakdown').innerHTML = result.categories.map(item => `
